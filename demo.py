from tkinter import ttk,Tk,PhotoImage,ROUND
import cv2
from tkinter import Canvas,filedialog,colorchooser,RIDGE,GROOVE,ROUND,Scale,HORIZONTAL
...
class FrontEnd:
    def __init__(self,master):
        ...
        self.logo=PhotoImage(file='python1.gif').subsample(5,5)
        print(self.logo)
        ttk.Label(self.frame_header,image=self.logo).grid(row=0,column=0,rowspan=1)
        ...
        self.canavas = Canvas(self.frame_menu,bg="gray",width=300,height=400)
        self.canavas.grid(row=0,column=1,rowspan=10)
    ...
    def crop_action(self):
        ...
        self.crop_end_x = 0
        self.crop_end_y = 0
        self.canavas.bind("<ButtonPress>",self.start_crop)
        self.canavas.bind("<B1-Motion>",self.crop)
        self.canavas.bind("<ButtonRelease>",self.end_crop)
    ...
    def crop(self,event):
        if self.rectangle_id:
            self.canavas.delete(self.rectangle_id)

        self.crop_end_x = event.x
        self.crop_end_y = event.y

        self.rectangle_id = self.canavas.create_rectangle(self.crop_start_y,self.crop_end_y,width=1)
    ...
    def text_action_1(self):
        ...
        ttk.Label(self.side_frame,text="Please enter a text").grid(row=0,column=2,padx=5,pady=5,sticky='sw')
    ...
    def text_action(self):
        ...
        self.canavas.bind("<ButtonPress>",self.start_crop)
        self.canavas.bind("<B1-Motion>",self.crop)
        self.canavas.bind("<ButtonRelease>",self.end_text_crop)
    ...
    def draw_action(self):
        ...
        self.refresh_side_frame()
        self.canavas.bind("<ButtonPress>",self.start_draw)
        self.canavas.bind("<B1-Motion>",self.draw)
    ...
    def draw(self,event):
        print(self.draw_ids)
        self.draw_ids.append(self.canavas.create_line(self.x,self.y,event.x,event.y,width=2,fill=self.color_code[-1],capstyle=ROUND,smooth=True))
  ...
  #      ttk.Label(self.side_frame,text="Averaging Blur").grid(row=0,column=2,padx=5,pady=5,sticky='sw')
  #      self.average_slider = ttk.Scale(self.side_frame,from_=0,to=256,orient='horizontal',command=self.averaging_action)
   #     self.average_slider.grid(row=1,column=2,padx=5,sticky='sw')
#
 #       ttk.Label(self.side_frame,text="Gaussian Blur").grid(row=2,column=2,padx=5,pady=5,sticky='sw')
  #      self.gaussian_slider = ttk.Scale(self.side_frame,from_=0,to=256,orient='horizontal',command=self.gaussian_action)
   #     self.gaussian_slider.grid(row=3,column=2,padx=5,sticky='sw')
    ...
    #    ttk.Label(self.side_frame,text="Median Blur").grid(row=4,column=2,padx=5,sticky='sw')
     #   self.median_slider = ttk.Scale(self.side_frame,from_=0,to=256,orient='horizontal',command=self.median_action)
    ...
    def adjust_action(self):
        ...
        self.saturation_slider=Scale(self.side_frame,from_=0,to_=20,resolution = 0.5,orient='horizontal',command=self.saturation_action)
    ...
    def display_image(self,image=None):
       ...
       #If image is not passed,We are going to show the recent edited_image
        ...
        #Converting BGR color scheme to RGB for tkinter canavas to remender image properly
        ...
        #if the image size is grater than the canavas,we are assigning new height and width to the image
        ...
        self.canavas.create_image(new_width/2,new_height/2, image=self.new_image)
